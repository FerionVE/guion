...
impl<E> dyn Widget<E>+'_ where E: Env {
    #[inline]
    pub fn is_type<T>(&self) -> bool where T: Statize<E> { //TODO purge downcast
        self.typeid() == T::_typeid()
    }

    #[inline]
    pub fn _downcast_ref<'s,'w,T>(&'s self) -> Option<&'s T> where T: Statize<E>+'w, Self: 'w {
        if self.is_type::<T>() {
    ...
    #[inline]
    pub fn downcast_ref<'s,'w,T>(&'s self) -> Option<&'s T> where T: Statize<E>+'w, Self: 'w {
    ...
    #[inline]
    pub fn _traitcast_ref<'s,'w,T>(&'s self) -> Option<&'s T> where T: Statize<E>+?Sized+'w, Self: 'w {
    ...
    #[inline]
    pub fn traitcast_ref<'s,'w,T>(&'s self) -> Option<&'s T> where T: Statize<E>+?Sized+'w, Self: 'w {
...
impl<E> dyn WidgetMut<E>+'_ where E: Env {
    ...
    #[inline]
    pub fn _downcast_mut<'s,'w,T>(&'s mut self) -> Option<&'s mut T> where T: Statize<E>+'w, Self: 'w {
        if self.is_type::<T>() {
            unsafe { Some(&mut *(self as *mut dyn WidgetMut<E> as *mut T)) }
        } else {
            None
        }
    }
    /// downcast the current widget to a specific implementation
    #[inline]
    pub fn downcast_mut<'s,'w,T>(&'s mut self) -> Option<&'s mut T> where T: Statize<E>+'w, Self: 'w {
        if self.is_type::<T>() {
    ...
    #[inline]
    pub fn _traitcast_mut<'s,'w,T>(&'s mut self) -> Option<&'s mut T> where T: Statize<E>+?Sized+'w, Self: 'w {
    ...
    /// this will definetly cause UB and delet ur computer
    #[inline]
    pub fn traitcast_mut<'s,'w,T>(&'s mut self) -> Option<&'s mut T> where T: Statize<E>+?Sized+'w, Self: 'w {
    ...
    pub fn _downcast_ref<'s,'w,T>(&'s self) -> Option<&'s T> where T: Statize<E>+'w, Self: 'w {
        if self.is_type::<T>() {
            unsafe { Some(&*(self as *const dyn WidgetMut<E> as *const T)) }
        } else {
            None
        }
    }
