...
pub trait Event<E> where E: Env {
    ...
    #[inline]
    fn query<'a,Q,S>(&'a self, query: &Q, path: &mut NewPathStack, stack: &S) -> Option<Q::Out<'a>> where Q: Query<E> + ?Sized, S: Queron<E> + ?Sized, Self: 'a {
    ...
    fn _query<'a,Q,S>(&'a self, builder: QueryStack<'_,'a,Q,E>, path: &mut NewPathStack, stack: &S) where S: Queron<E> + ?Sized, Self: 'a;
    ...
    #[deprecated]
    #[inline]
    fn query_std_event_mode<'a,S>(&'a self, path: &mut NewPathStack, stack: &S) -> Option<StdEventMode<E>> where S: Queron<E> + ?Sized, Self: 'a {
    ...
    #[inline]
    fn query_variant<'a,V>(&'a self, path: &mut NewPathStack, stack: &(impl Queron<E> + ?Sized)) -> Option<&'a V> where V: Clone + 'static, Self: 'a {
...
pub trait EventDyn<E> {
    fn _query_dyn<'a>(&'a self, builder: QueryStack<'_,'a,DynQuery,E>, path: &mut NewPathStack, stack: &(dyn QueronDyn<E>+'_));
    fn ts_dyn(&self) -> u64;
...
impl<E> Event<E> for dyn EventDyn<E> + '_ where E: Env {
    ...
    fn _query<'a,Q,S>(&'a self, mut builder: QueryStack<'_,'a,Q,E>, path: &mut NewPathStack, stack: &S) where S: Queron<E> + ?Sized, Self: 'a {
        self._query_dyn(builder.fork_dyn(),path,stack.erase())
    }
    #[inline]
    fn erase<'s,'ss>(&'s self) -> &'s (dyn EventDyn<E>+'ss) where 'ss: 's, Self: 'ss {
        self
