...
pub trait RenderStdWidgets<E>: Render<E> where E: Env {
    ...
    #[inline]
    fn render_text<Q>(&mut self, text: &str, align: (f32,f32), props: &Q, c: &mut E::Context<'_>) where for<'r> ERenderer<'r,E>: RenderStdWidgets<E>+'r, Q: Queron<E> + ?Sized {
    ...
    fn render_preprocessed_text<Q>(&mut self, text: &ETextLayout<E>, inner_offset: Offset, props: &Q, c: &mut E::Context<'_>) where Q: Queron<E> + ?Sized;

    fn set_cursor_specific(&mut self, cursor: &ESCursor<E>, c: &mut E::Context<'_>);

    /// Set the cursor to the cursor derived from style
    #[inline]
    fn set_cursor<Q>(&mut self, props: &Q, c: &mut E::Context<'_>) where Q: Queron<E> + ?Sized {
