...
pub trait WidgetID: AsRefMut<Self> + Clone + PartialEq + Sized + Debug + 'static { //should also implement Eq + Hash + Send
    ...
    #[inline]
    fn is_hovered<E: Env>(&self, c: &E::Context<'_>) -> bool where for<'a> E::Context<'a>: CtxStdState<'a,E>, for<'a> EPressedKey<'a,E>: PressedKey<E>, Self: AsRefMut<E::WidgetID> {
        c.state().is_hovered(self.as_ref())
    }
    #[inline]
    fn is_focused<E: Env>(&self, c: &E::Context<'_>) -> bool where for<'a> E::Context<'a>: CtxStdState<'a,E>, for<'a> EPressedKey<'a,E>: PressedKey<E>, Self: AsRefMut<E::WidgetID> {
